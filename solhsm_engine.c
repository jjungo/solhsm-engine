...
static EVP_PKEY* solhsm_load_private_key(ENGINE *e, const char *key_id,
	...
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm load (dummy) private key \
	...
	/*Forge frame*/
	solhsm_rsa_key_payload* rsa_key = solhsm_forge_rsa_key_payload(
		0, 
	...
	solhsm_network_frame_container* c_s = solhsm_forge_container(
		1, 
		RSA_KEY, 
		solhsm_get_rsa_key_payload_size(rsa_key)); 
		

	/*Send frame*/
	solhsm_network_send(client, c_s, rsa_key);
	/*wait a respons*/
	solhsm_network_frame_container *c_r = 
		malloc(sizeof(solhsm_network_frame_container));
		
	void* ptr_key_recv = solhsm_network_receive(client, c_r);
	
	/*process respons*/
	solhsm_rsa_key_payload *rsa_key_rcv = (solhsm_rsa_key_payload *)ptr_key_recv; 
	
	/*check respons status*/
	if(rsa_key_rcv->key_type == RSA_KEY_ERROR){
		//error in respons
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm load privkey rcv msg is in ERROR \
			state : %s]",rsa_key_rcv->key_data);
		...
		/*wrong key type*/
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm load privkey rcv msg is a public key,\
			we are waiting a dummy key]");
		return NULL; 
	}
	...
	{	
		...
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm load privkey failed]");
		return NULL; 
	}
}
...
{		
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm encrypt is called with hsm is = %s \
		and key_id = %i]", init_params.solhsm_hsm_ip, 
		init_params.solhsm_key_id);
	...
	solhsm_rsa_std_payload* rsa_to_send = solhsm_forge_rsa_std_payload(
	flen, 
	init_params.solhsm_key_id, 
	padding, 
	RSA_PUB_ENC, 
	(unsigned char*) from); //cast to work with in frame forgery
	/*forge container*/
	solhsm_network_frame_container* c_to_send = solhsm_forge_container(
	1,
	RSA_STD, 
	solhsm_get_rsa_std_payload_size(rsa_to_send)); 
	
	/*send */
	solhsm_network_send(client, c_to_send, rsa_to_send);
	
	/*forge container to receive*/
	solhsm_network_frame_container* c_rcv = malloc(
	sizeof(solhsm_network_frame_container)); 
	
	/*receive*/
	void* ptr_rcv = solhsm_network_receive(client,c_rcv);
	/*cast*/
	solhsm_rsa_std_payload *rsa_rcv = (solhsm_rsa_std_payload *)ptr_rcv;
	
	if(rsa_rcv->operation == RSA_STD_ERROR){
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
			"[OpenSSL ENGINE solhsm HSM reply error in pub_enc : %s]",
			rsa_rcv->data);
		return -1; 
	}
...
{	
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm verify is called with hsm is = %s and\
		key_id = %i]", init_params.solhsm_hsm_ip,
		init_params.solhsm_key_id);
	...
	if(rsa_rcv->operation == RSA_STD_ERROR){
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
			"[OpenSSL ENGINE solhsm HSM reply error in pub_dec : %s]", 
			rsa_rcv->data);
		return -1; 
	}
...
{	
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
	"[OpenSSL ENGINE solhsm decrypt is called with hsm is = %s and \
		key_id = %i]", init_params.solhsm_hsm_ip, 
		init_params.solhsm_key_id);
	...
	if(rsa_rcv->operation == RSA_STD_ERROR){
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm HSM reply error in priv_dec : %s]", 
		rsa_rcv->data);
		return -1; 
	}
	/*give value to openssl*/
  ...
  unsigned char *to, RSA *rsa, int padding)
{
	...
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
	"[OpenSSL ENGINE solhsm sign is called with hsm is = %s and key_id = %i]",
	 init_params.solhsm_hsm_ip, init_params.solhsm_key_id);
	/*init zmq crypto*/
    zcert_t *client_cert = zcert_load (init_params.solhsm_priv_cert_path);
    zcert_t *server_cert = zcert_load (init_params.solhsm_pub_cert_path);
	...
	if(rsa_rcv->operation == RSA_STD_ERROR){
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
			"[OpenSSL ENGINE solhsm HSM reply error in priv_enc : %s]",
			rsa_rcv->data);
		return -1; 
	}
...
static int solhsm_finish(ENGINE *e)
	{
	if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
		"[OpenSSL ENGINE solhsm finish called ]");
	closelog ();
	return 1;
	}
...
static int solhsm_ctrl(ENGINE *e, int cmd, long i, void *p, void (*f)(void))
	...
	switch (cmd) {
		...
		else init_params.solhsm_set_debug = 0; 
		/*Start logging */
	...
	case CMD_ZMQ_SERVER_PUB_CERT_PATH:
		...
		if(init_params.solhsm_set_debug)syslog(LOG_INFO,
			"[OpenSSL ENGINE solhsm command public cert path is set to %s]",
			 (const char *)p);
		return 1;
	case CMD_ZMQ_CLIENT_PRIV_CERT_PATH:
		...
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
			"[OpenSSL ENGINE solhsm command private cert path is set to %s]",
			(const char *)p);
		return 1;
	case CMD_ZMQ_SERVER_IP:
		...
		if(init_params.solhsm_set_debug)syslog(LOG_INFO,
			"[OpenSSL ENGINE solhsm command hsm ip is set to %s]", 
			(const char *)p);
		return 1;
	case CMD_KEY_ID:
		init_params.solhsm_key_id = (int)i;
		if(init_params.solhsm_set_debug)syslog(LOG_INFO, 
			"[OpenSSL ENGINE solhsm command key id is set to %i]",
			 (int)i);
		return 1;
	default:
		break;
